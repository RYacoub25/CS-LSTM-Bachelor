...
class ContextualSocialLSTM(nn.Module):
    def __init__(self, input_size=9, context_dim=15, hidden_size=256, output_size=2, intention_dim=1, num_layers=2):
        ...
        self.intention_fc = nn.Linear(intention_dim, hidden_size)  # Expect intention_dim to be 1
        self.output_fc = nn.Linear(hidden_size * 2 + hidden_size, output_size)  # Include intention size in input to output
    ...
    def forward(self, hist, nbrs, mask, context, intention):
        ...
        # Process intention through a fully connected layer
        intention_emb = self.intention_fc(intention.unsqueeze(1))  # Ensure intention is of shape (batch_size, 1)
        intention_emb = intention_emb.squeeze(1)  # Remove the extra dimension if necessary
        
        combined = torch.cat([ego_final + attended_nbrs, attended_context, intention_emb], dim=1)
